...
5. Using the following template, sort the dataframe in descending order of price. Then, print it. Do you see the house with the highest selling price at the top?
...
```{r, echo = TRUE, eval = FALSE}
price_glm <- glm(formula = XXX ~ XXX + XXX + XXX + XXX + XXX,
                 data = XXX)
```
...
1. Which zipcode has the highest percentage of houses on the waterfront? (Hint: group by zipcode, calculate the percentage of houses on the waterfront using `mean()`, then sort the data in descending order)

2. Which house had the highest price to living space ratio? To answer this, create a new variable called `price_to_living` that takes `price / living_sqm`. Then, sort the data in descending order of this variable, and select the first row with `slice()`!
...
5. Create a regression object predicting the amount of living space (`living_sqm`) as a function of `bedrooms`, `bathrooms`, `waterfront`, and `built_yr`. Explore the object with `names()` and `summary()`. Which variables seem to predict living space?
