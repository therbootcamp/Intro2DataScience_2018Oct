...
- `formula = y ~ x1 + x2 + ...`
- `formula = y ~ .` (use *all* variables in `data`)

Use `glm()` (or `lm()`) to create a linear model (regression) object


```{r}
# Create regression model predicting income
income_glm <- glm(formula = income ~ age + height + children,
                data = baselers)

# What is in this object?
names(income_glm)[1:10] # only showing first 10

# Print coefficients
income_glm$coefficients
```
...
tidy(income_glm) # Return a 'tidy' dataframe
```
