...
1. Open your `baselrbootcamp` R project. It should already have the folders `1_Data` and `2_Code`. Inside of the `2_Code` folder, you should have all of the data sets listed in the Datasets section above!
...
6. Finally, we want the data in the "long", instead of the current "wide" format. In long formats variables occupy different rows rather than columns. To this using the `gather()`-function. Replace the XXs below below to create the `financial_long` data frame. Hint: The first two arguments to the `gather`-function specify the names of the new variables, the third and fourth specify the names of the variables whose format you would like to change (minus in front is intentional). Check out the example in the `?gather`-help file.
