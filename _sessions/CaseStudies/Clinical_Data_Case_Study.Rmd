...
## read in datasets
trial_df <- read_csv("_sessions/CaseStudies/1_Data/dimarta_trial.csv")
demographics_df <- read_csv("_sessions/CaseStudies/1_Data/dimarta_demographics.csv")
biomarker_df <- read_csv("_sessions/CaseStudies/1_Data/dimarta_biomarker.csv")
```
...
1. Use the following template to create boxplots showing the relationship between study arm and histamine change.
...
3. Create the same plot as above, but instead of analysing study arm, try analysing gender. What do you find? Did one gender have better histamine improvements than the other?
...
1. Create a regression model predicting final histamine levels as a function of all variables. Which variables predict final histamine levels?

2. Now do the same predicing histamine *change*, but *don't* include final histamine levels. Do you get the same results as above? 

3. Try running the same model as above but *do* include final histamine levels. What do you find?