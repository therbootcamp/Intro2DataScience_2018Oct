...
5. Using the same code structure, load the `trial_act_demo.csv` data as a new dataframe called `trial_act_demo`
...
In this practical you will do many operations on dataframes. If you can, try to Remember when using `dplyr`, you can chain multiple functions together with the pipe `%>%`. When giving your answers to the questions in this practical, see how many operations you can chain with the pipe!
...
1. Using the `filter()` funcion, filter only the rows from males (Hint: `gender_char == "male"`)
...
1. Using the `spread()` function, create a new dataframe called `CD4_long` that shows the `CD4_wide` data in the 'long' format. To do this, use the following template. Set the grouping column to `time` and the new data column to `value`. 
...
2. Add a new column to `trial_act` called `drugs_char` which is `"User"` when drug == 1, and `"Non User"` when drug == 0.
